...
const ListPlant = () => {
  ...
  const filteredPlants = useMemo(() => {
    return (allPlant || []).filter(
      (plant) =>
        plant.name.toLowerCase().includes(inputData.toLowerCase()) ||
        plant.description.toLowerCase().includes(inputData.toLowerCase())
    );
  }, [allPlant, inputData]);
  ...
  return (
    <section className="mx-auto sm:px-6 lg:px-8 py-24 justify-center">
      <div className="relative w-full lg:max-w-4xl mx-auto bg-white rounded-full mt-2 mb-10">
        <input
          placeholder="Nhập tên, mô tả cây cần tìm kiếm..."
              ...
              >
                ...
                <div className="p-6">
                  <div className="mb-2 flex items-center justify-between">
                    ...
                    <p className="block font-sans text-base font-medium leading-relaxed text-green-500 antialiased">
                      {renderTypePlant(plant.type)}
                    </p>
                  </div>
                  <p className="block font-sans text-sm font-normal leading-normal text-gray-700 antialiased opacity-75">
                    {plant.description.length > MAX_DESCRIPTION_LENGTH
                ...
                <div className="p-6 pt-0">
                  <button
                    ...
                    type="button"
                    onClick={() => navigate(`/plant/recommend/${plant.id}`)}
                  >
                    Chi tiết
                  </button>
                </div>
